...
class Lingotek_Filters_Columns extends PLL_Admin_Filters_Columns {
		...
		foreach ( $this->model->get_languages_list() as $language ) {
            ...
            if ($is_pll_item) {
                if (!in_array($language_key, $post_languages)) {
                    $post_languages[] = $language_key;
                }
                if (!in_array($language_key, $page_languages)) {
                    $page_languages[] = $language_key;
                }
            }
		}
        update_user_meta($current_user_id,'manageedit-postcolumnshidden', $post_languages);
        update_user_meta($current_user_id,'manageedit-pagecolumnshidden', $page_languages);
	...
	private function print_word_limit_error() {
		...
		printf( 
			...
			esc_html__( 'Processed word limit exceeded. Please contact your local administrator or Lingotek Client Success ', 'lingotek-translation' ) . '<a href="' . esc_url( 'mailto:sales@lingotek.com' ) . '">' . esc_html__( '(sales@lingotek.com)', 'lingotek-translation' ) . '</a>' . esc_html__( ' for assistance.', 'lingotek-translation' )
		);
	...
	protected function _column( $type, $column, $object_id, $custom_data = null ) {
		...
		$get_action         = sanitize_text_field( filter_input( INPUT_GET, 'action' ) );
		$inline_lang_choice = sanitize_key( filter_input( INPUT_POST, 'inline_lang_choice' ) );
		...
		$id = ( ( $inline && $lang->slug !== $this->model->get_language( sanitize_key( filter_input( INPUT_POST, 'old_lang' ) ) )->slug ) ?
